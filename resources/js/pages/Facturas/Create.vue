            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  {{
                    formatCurrency(
                      producto.precio_unitario * detalleFactura.cantidad
                    )
                  }}
                </td>
                <td>
                  {{
                    formatCurrency(
                      producto.precio_unitario * detalleFactura.cantidad * 0.19
                    )
                  }}
                </td>
                <td>
                  {{
                    formatCurrency(
                      producto.precio_unitario * detalleFactura.cantidad * 1.19
                    )
                  }}
                </td>
...
export default {
  ...
  methods: {
    ...
    agregarDetalleFactura() {
      let valorDetalle;
      valorDetalle =
        this.producto.precio_unitario * this.detalleFactura.cantidad * 1.19;
      ...
      let ay = {
        valor_total: valorDetalle,
        cantidad: this.detalleFactura.cantidad,
        producto_id: this.producto.id,
        bodega_id: this.producto.id,
      };
      this.detalles.splice(0, 0, ay);
      this.limpiarCampos();
      this.factura.valor_total += valorDetalle;
      localStorage.setItem(
        "detalleFacturas",
        JSON.stringify(this.detalleFacturas)
      );
    },
    ...
    registrarDetalleFactura(item) {
      ...
      this.axios.post("/api/detalle-facturas", detFac);
    },
    guardarFactura() {
